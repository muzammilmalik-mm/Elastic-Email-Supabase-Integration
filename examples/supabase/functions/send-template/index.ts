...
serve(async (req) => {
    ...
    try {
        ...
        // Parse request body
        const { to, templateName, templateData, cc, bcc, replyTo } = await req.json();

        // Validate required fields
        if (!to || !templateName) {
            throw new ValidationError(
                'Missing required fields: to and templateName are required'
            );
        }
        ...
        let statusCode = 500;
        let errorMessage = 'Failed to send template email';
